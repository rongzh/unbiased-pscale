...
#read data from file
dat = np.loadtxt(fname='UpUs-Au.md', delimiter='|', skiprows=2)
print dat
Up = dat[:,0]
Us = dat[:,1]
plt.plot(Up,Us,'rx')
...
#set Au density
rho1original = 19.32 #unit grams/cm3
...
m_cell = 197*4/atompermol # (g/unitcell)
...
set_const()
V_a = 197*param_d['const']['Natom']/atompermol/rho2 #unit Ang^3/unitcell
print "V_a" , V_a
# 1eV/ang^3 = 160.217657GPa, 1eV = 1.6021764e-19Joules, 1Ang3 = e-30m^3
fei_report = [67.850, 167,6.00,np.log(170),np.log(2.97),0.6]
set_dic(fei_report)
...
print "test", findroot(300,V_a,param_d,E2),findroot(3000,V_a,param_d,E2)
#find the temperature
result = []
for ind in range(5):
  T_root = optimize.brentq(findroot, a=300,b=3000,args = (V_a[ind+3],param_d,E2[ind+3]),full_output = False)
  print "root", T_root
...
def Jam_fit(T_a,P,param_d):
    return P - (19.2827 * 3.215) * energy_Jam_mgd_powlaw(197*param_d['const']['Natom']/atompermol/19.2827,T_a,param_d)
...
####
rho0 = 19.2827/10**24 #grams/Ang^3
rho2 = rho0*Us/(Us-Up) #unit grams/Ang^3
V_0 = 197*param_d['const']['Natom']/atompermol/rho0 #unit Ang^3/unitcell


#read Jam's data here and fit the vinet model for 300K
dat = np.loadtxt("FigAu.txt", delimiter = ",", skiprows = 1)
V300_a = (1 - dat[:,2]) * V_0
P300_a = dat[:,1]
...
def findT(T, Pth,rho2):
    atompermol = 6.022*(10**23) #at/mol
    rho0 = 19.2827 #unit g/cm^3
    V_0 = 197*param_d['const']['Natom']/atompermol/rho0*1e24 #unit Ang^3/unitcell
    g0 = 3.215
    thetaD = 170 #unit K
...
print "findT: ", findT(300,Pth,rho2*1e24), findT(20000,Pth,rho2*1e24)
...
for ind in range(len(Pth)):
    T_root = optimize.brentq(findT, a=300,b=1000000,args = (Pth[ind],rho2[ind]*1e24),full_output = False)
    Tfit[ind] = T_root
print Tfit
