...
def set_const(param_dic):
    ...
    param_dic['const']['fcv'] = 0.12786/0.12664#the ratio of Cvmax/DP
    param_dic['const']['Atmospheric_P'] = 101325*10**(-9) #unit GPa
    param_dic['const']['mol_mass'] = 195.0 #unit g/mol
...
def set_Jam_params(param_dic): #set parameters that Jamieson reported 
    param_dic['Jam_ref']['gamma0'] = 2.4 #reported by Jam
    param_dic['Jam_ref']['theta0'] = 200.0 #unit K
    param_dic['Jam_ref']['q'] = 1.0 #const
    param_dic['Jam_ref']['Cb'] = 3.574 #intercept for Up-Us line in fig1
    param_dic['Jam_ref']['S'] = 1.582 #slope for Up-Us line in fig1
    param_dic['Jam_ref']['rho0'] = 21.4449 #g/cm^3
...
#read data from file
shock_dat = np.loadtxt(fname='UpUs-Pt.md', delimiter='|', skiprows=3)
print shock_dat
Up = shock_dat[:,0]
Us = shock_dat[:,1]
plt.plot(Up,Us,'rx')
...
Table4_dat = np.loadtxt(fname='Table4-Pt.md', delimiter='|', skiprows=1)
...
Up_sample = np.linspace(0.01,0.908,100)
...
Fig1_Pt_dat = np.loadtxt(fname='Fig1-Pt.md', delimiter=',', skiprows=1)
Fig1_Pt_T = Fig1_Pt_dat[:,0]*1000.0
Fig1_Pt_P = Fig1_Pt_dat[:,1]
f = interp1d(Fig1_Pt_P, Fig1_Pt_T)
