...
def lsqfun(p_eos,volume,temperature,Natom,P):
	set_dic(p_eos)
	resid_scl = MGD_PowerLaw(volume, temperature, param_d) - P
	return resid_scl
...
def fitwrapper(p_eos, param_d, data_d, prior_d = None):
	error = data_d['P_err_tot']
	volume = data_d['V']
	temperature = data_d['T']
	P = data_d['P']
	set_dic(p_eos)
	resid_scl = (MGD_PowerLaw(volume, temperature,param_d) - P)/error
	if len(prior_d) != 0:
		resid_scl = np.hstack(resid_scl,(p_eos - prior_d['paras'])/prior_d['width'])
	return resid_scl
...
data_d = collections.OrderedDict()
prior_d = collections.OrderedDict()
#read data
...
#set Fei's parameters into dictionary
fei_report = [60.38, 277,5.08,np.log(230),np.log(2.72),0.5]

param_d['Natom'] = Natom_Pt
...
data_d['T'] = T_Pt
data_d['V'] = V_Pt
data_d['P_err_tot'] = error

#plot raw data read from file
...
guess = [60.38,277,5.08,np.log(230),np.log(2.72),0.5]
popt = optimization.leastsq(fitwrapper,guess[:],args=(param_d,data_d,prior_d), ftol = Fei_errorbar,full_output = 1)
print popt
cov = popt[1]
mean = popt[0]
print mean, cov
print cov[3][3], cov[4][4]
...
pressure300 = MGD_PowerLaw(V,300*np.ones(len(V)),param_d)
pressure1473 = MGD_PowerLaw(V,1473*np.ones(len(V)),param_d)
pressure1873 = MGD_PowerLaw(V,1873*np.ones(len(V)),param_d)
...
for i in range(1,9):
	...
	pressure1873 = MGD_PowerLaw(V,1873*np.ones(len(V)), param_d)
	print "a is: " ,a
	print "300K: ", pressure300
	print "1473K: ", pressure1473
	print "1873K: ", pressure1873
